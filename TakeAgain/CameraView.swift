                                                ...
                                                Button(action: {
                                                    ...
                                                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                                                        PHPhotoLibrary.shared().performChanges({
                                                            PHAssetChangeRequest.creationRequestForAssetFromVideo(atFileURL: cameraManager.outputURL!)
                                                        }) { success, error in
                                                            if success {
                                                    ...
                                                    TapGesture()
                                                        .onEnded {
                                                            if !hasSeenDeleteWarning {
                                                                ...
                                                                deleteAndSaveToRecentlyDeleted()
                                                            }
                                                        }
                                                )
                                                .gesture(
    ...
    func fetchLatestPhotoThumbnail() {
        ...
        imageManager.requestImage(for: asset, targetSize: targetSize, contentMode: .aspectFill, options: options) { image, _ in
            if let image = image {
                ...
                print("✅ 최근 썸네일 불러오기 성공")
            }
        }
    }

    func deleteAndSaveToRecentlyDeleted() {
        cameraManager.stopRecording()
        recordingTime = 0
        isRecording = false

        guard let url = cameraManager.outputURL else { return }
